...
class Chairman(Agent):
    def __init__(
        self, client_factory: Callable, executives: list[SME], name: str = "Chairman"
    ) -> None:
        # Construct the user_prompt string with details of the executives
        self.user_prompt = self.update_user_prompt(executives)

        system_prompt = "Answer with only the name and nothing else."
    ...
    def decide_if_meeting_over(self, _transcript: str) -> bool:
        return False
    ...
    def decide_next_speaker(self, transcript: str) -> SME:
        while True:
            ...
            next_executive = next(
                (exec for exec in self.executives if exec.name == next_speaker),
                None,
            )
